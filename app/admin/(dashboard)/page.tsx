  ...
  return (
    <div className="flex animate-in flex-col gap-6 fade-in slide-in-from-bottom-2 duration-500">
      ...
      {/* One outlined strip, divided by hairlines — not four separate card
          ...
          admin.css). */}
      <div className="glass-outline flex flex-col divide-y divide-border overflow-hidden rounded-2xl sm:flex-row sm:divide-x sm:divide-y-0">
        <StatTile
      ...
      <Card>
        ...
        <CardContent className="flex flex-col divide-y divide-border overflow-hidden p-0 sm:flex-row sm:divide-x sm:divide-y-0">
          ...
          <CardContent className="flex flex-1 flex-col gap-1">
            {loadingEnquiries ? (
              ...
              recent.map((e, i) => (
                <div
                  key={e.id}
                  className={`flex min-w-0 items-start justify-between gap-2 py-3 ${i > 0 ? "border-t border-border" : ""}`}
                >
            ...
            <Link
              ...
              className="mt-auto pt-4 text-xs font-medium text-[var(--wine)] transition-colors hover:text-[var(--wine-deep)] hover:underline"
            >
              View all enquiries →
            </Link>
          </CardContent>
